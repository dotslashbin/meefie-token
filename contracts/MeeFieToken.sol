...
contract MeefieToken is Context, IERC20, IERC20Metadata, Ownable, IERC20Errors {
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        if (!isExcludedFromFee[from] && !isExcludedFromFee[to]) {
            if (to == uniswapV2Pair) { // Sell transaction
                fees = value * 2 / 100; // 2% sell tax
                burnAmount = value * 1 / 100; // 1% burn on sell
            } else if (from == uniswapV2Pair) { // Buy transaction
                fees = value * 1 / 100; // 1% buy tax
            }

            if (fees > 0) {
                _update(from, address(this), fees); // Collect fees
            }
